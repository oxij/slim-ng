	  ...
	  session_name(""), session_exec("")
{
	...
	if (mode == Mode_Lock) {
		...
		X = Cfg::absolutepos(
			cfgX, XWidthOfScreen(ScreenOfDisplay(Dpy, Scr)), image->Width());
		Y = Cfg::absolutepos(
			cfgY, XHeightOfScreen(ScreenOfDisplay(Dpy, Scr)), image->Height());
	}
...
{
	/* Create window */
	Win = XCreateSimpleWindow(Dpy, Root, X, Y, image->Width(), image->Height(),
...
void Panel::ShowText()
{
	...
	if (welcome_x >= 0 && welcome_y >= 0) {
		SlimDrawString8(draw, &welcomecolor, welcomefont, welcome_x, welcome_y,
			welcome_message, &welcomeshadowcolor, shadowXOffset, shadowYOffset);
	}
...
string Panel::getSession() { return session_exec; }
...
void Panel::SlimDrawString8(XftDraw * d, XftColor * color, XftFont * font,
	int x, int y, const string & str, XftColor * shadowColor, int xOffset,
	int yOffset)
{
	int calc_x = 0;
	int calc_y = 0;
...
const string & Panel::GetName(void) const { return NameBuffer; }

const string & Panel::GetPasswd(void) const { return PasswdBuffer; }
